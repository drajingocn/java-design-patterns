...
##### AUT-pipeline v3
##### This script runs the tests for the project as a whole, it is run if the "Module-flag" is set to false in autout.yml
...
run_gp() {
    ...
    do
        java -jar general-patterns-1.3.1.jar --project.src=${SOURCE} --project.out=${OUTPUT} --spring.profiles.active=${profile}
    done
}
...
echo -e "PIPELINE: Command: haha$1haha\n\n"
...
# Declare the arrays that hold the project build, run and run-tests commands
declare -a BUILD
declare -a RUN
declare -a TEST
...
start_test # Run the test commands from project root

i=0 # Variable i is index for the modules as defined in autout.yml
while [[ ${i} -lt ${#SRC[@]} ]] # Loop over all the modules to run GeneralPatternsAnalyzer, and if needed, GeneralPatterns
do
...
i=0 # Variable i is index for the modules as defined in autout.yml
while [[ ${i} -lt ${#SRC2[@]} ]] # Loop over all the modules
do
    cd ${SRC2[$i]}
